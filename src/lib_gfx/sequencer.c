...
KV_SECTION_META kv_meta_t seq_info_kv[] = {
...
static int8_t _run_step( bool select_current_step ){
	...
	else if( seq_select_mode == SEQ_SELECT_MODE_NEXT ){

		seq_current_step++;
	}
	else if( seq_select_mode == SEQ_SELECT_MODE_RANDOM ){
...
PT_BEGIN( pt );
	...
	THREAD_WAIT_WHILE( pt, seq_time_mode == SEQ_TIME_MODE_STOPPED );
	...
	while( !sys_b_is_shutting_down() ){
		...
		if( is_vm_sync_follower() ){
	    	...
	    	THREAD_WAIT_WHILE( pt, 
	    		( seq_time_mode == SEQ_TIME_MODE_MANUAL ) &&
	    		!is_charging() &&
	    		!sys_b_is_shutting_down() &&
	    		( seq_trigger == FALSE ) );
	    ...
	    else if( seq_time_mode == SEQ_TIME_MODE_BUTTON ){

	    	THREAD_WAIT_WHILE( pt, 
	    		( seq_time_mode == SEQ_TIME_MODE_BUTTON ) &&
	    		!is_charging() &&
	    		!sys_b_is_shutting_down() &&
	...
	if( sys_b_is_shutting_down() && ( seq_time_mode != SEQ_TIME_MODE_STOPPED ) ){

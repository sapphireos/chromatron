...
void ldr_run_app( void ){
    ...
    trace_printf("image header @ 0x%x: %x %u %x %x\n",
        addr,
    	image_header.magic,
    ...
    for( uint8_t i = 0; i < image_header.segment_count; i++ ){
        ...
        if( ( section_header.load_addr >= SOC_DROM_LOW ) &&
            ( section_header.load_addr < SOC_DROM_HIGH ) ){

            trace_printf("DROM: 0x%08x %u bytes\n", section_header.load_addr, section_header.data_len);

            drom_addr       = addr;
            drom_load_addr  = section_header.load_addr;
            drom_size       = section_header.data_len;
        }
        if( ( section_header.load_addr >= SOC_IROM_LOW ) &&
            ( section_header.load_addr < SOC_IROM_HIGH ) ){

            trace_printf("IROM: 0x%08x %u bytes\n", section_header.load_addr, section_header.data_len);

            irom_addr       = addr;
            irom_load_addr  = section_header.load_addr;
            irom_size       = section_header.data_len;
        }
    ...
    // cache and MMU loading cribbed from bootloader_utility.c:set_cache_and_start_app
    ...
    */
    ...
    trace_printf("d mmu set paddr=%08x vaddr=%08x size=%d n=%d\n",
    ...
    trace_printf("i mmu set paddr=%08x vaddr=%08x size=%d n=%d\n",

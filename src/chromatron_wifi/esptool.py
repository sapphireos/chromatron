...
class ESPROM:
    ...
    def flash_begin(self, size, offset):
        ...
        t = time.time()
        print "BEGIN"
        print size, num_sectors, head_sectors, sector_size
        print erase_size, num_blocks, ESPROM.ESP_FLASH_BLOCK, offset
    ...
    def flash_block(self, data, seq):
        ...
        print "LOAD", seq, len(data), ord(result[0]), ord(result[1]), '%x' % (ESPROM.checksum(data))
        if result != "\0\0":
...
def write_flash(esp, args):
    ...
    print flash_mode, flash_size_freq

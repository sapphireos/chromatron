...
# local tests:
PYTHONPATH=. python3 -m pytest -n auto --cov=chromatron --cov-config=tests/.coveragerc --cov-report=html tests/compiler/* -m local -v -x
...
# PYTHONPATH=. python3 -m pytest -n auto tests/compiler/test_code_gen.py::TestCompilerLocal::test_pixref_load -m local -v -x

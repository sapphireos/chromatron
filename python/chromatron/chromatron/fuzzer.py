...
from .code_gen import parse, compile_text, OPT_LEVELS
...
def test_program(fx_file, opt_passes=None):
    global selected_opt_passes

    if opt_passes is None:
        opt_passes = selected_opt_passes
    ...
    with open(fx_file, 'r') as f:
        ...
        print(f'Testing {fx_file} with opt: {opt_passes}')

        try:
            try:
                prog = compile_text(code, opt_passes=opt_passes)
...
def test_programs(target_dir='fuzzer', parallel=True, opt_passes=[]):
    ...
    selected_opt_passes = opt_passes
    ...
    if parallel:
        ...
        for fx_file in files:
            ...
            test_program(fx_file, opt_passes=opt_passes)
